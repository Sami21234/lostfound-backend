...
// 🔧 NEW: Handle found item reports with file upload (BEFORE route redirects)
app.post('/api/report-found', upload.single('image'), async (req, res) => {
  try {
    ...
    // Create found item with image path
    const foundItem = new FoundItem({
      ...
      imageUrl: req.file ? `/uploads/${req.file.filename}` : null // Store relative path
    });
...
// 🔧 NEW: Handle lost item reports with file upload (BEFORE route redirects)
...
// Original routes (keep these for other endpoints)
...
app.listen(PORT, () => {
  ...
  console.log(`   POST /api/report-lost (with file upload)`);
  console.log(`   POST /api/report-found (with file upload)`);
